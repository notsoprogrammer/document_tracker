...
class AddLocationalZoningScreen extends StatefulWidget {
  ...
  @override
  State<AddLocationalZoningScreen> createState() => _AddLocationalZoningScreenState();
}

class _AddLocationalZoningScreenState extends State<AddLocationalZoningScreen> {
  ...
  String _generateCode() {
    final prefix = typeMapping[selectedType] ?? 'LOC';
  ...
  Future<void> _takePicture() async {
    ...
    if (kIsWeb) {
      ...
      _webFileBytes[fileName] = scannedBytes;
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text("Add Locational / Zoning"),
          flexibleSpace: Container(decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFF80DEEA), Color(0xFF26C6DA), Color(0xFF00ACC1)], begin: Alignment.topLeft, end: Alignment.bottomRight))),
                ...
                Card(
                  ...
                  child: Padding(padding: const EdgeInsets.all(16.0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    ListTile(
                      ...
                      title: Text(codeController.text, style: Theme.of(context).textTheme.bodyLarge?.copyWith(fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.primary)),
                      trailing: ElevatedButton.icon(
                        onPressed: _isSaving ? null : () async {
                          final date = await showDatePicker(context: context, initialDate: selectedCalendarDate ?? DateTime.now(), firstDate: DateTime(2020), lastDate: DateTime.now().add(const Duration(days: 365)));
                          if (date != null && mounted) {
                            ...
                            if (time != null && mounted) {
                              ...
                              final endDate = await showDatePicker(context: context, initialDate: date, firstDate: date, lastDate: date.add(const Duration(days: 365)), helpText: 'Set end date (optional — cancel to skip)');
                              setState(() { selectedCalendarDate = startDateTime; selectedCalendarEndDate = endDate != null ? DateTime(endDate.year, endDate.month, endDate.day, 23, 59) : null; });
                            }
                          }
                        },
                        label: Text(selectedCalendarDate != null ? selectedCalendarEndDate != null ? "${DateFormat('MM/dd').format(selectedCalendarDate!)} – ${DateFormat('MM/dd').format(selectedCalendarEndDate!)}" : " ${DateFormat('MM/dd/yy hh:mm a').format(selectedCalendarDate!)}" : "Set to Calendar"),
                        style: ElevatedButton.styleFrom(backgroundColor: selectedCalendarDate != null ? Colors.green : Theme.of(context).colorScheme.secondary, foregroundColor: Theme.of(context).colorScheme.onSecondary),
                      ),
                    ),
                    ...
                    TextField(
                      ...
                      buildCounter: (context, {required currentLength, required isFocused, maxLength}) {
                        ...
                        return Text('$words / 6 words', style: TextStyle(fontSize: 12, color: words >= 6 ? Colors.orange[700] : Colors.grey));
                      },
                    ),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: locationalZoningTypes.map((type) => DropdownMenuItem<String>(value: type, child: Text(type))).toList(),
                      onChanged: _isSaving ? null : _onTypeChanged,
                    ),
                    const SizedBox(height: 12),
                    TextField(controller: descriptionController, enabled: !_isSaving, decoration: InputDecoration(labelText: "Description (optional)", hintText: "Project title, lot number, and other details", border: const OutlineInputBorder(), filled: true, fillColor: Theme.of(context).colorScheme.surface), maxLines: 3),
                    const SizedBox(height: 12),
                    InkWell(
...
class _WordLimitFormatter extends TextInputFormatter {
  ...
  @override
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    final words = newValue.text.trim().isEmpty ? 0 : newValue.text.trim().split(RegExp(r'\s+')).length;
