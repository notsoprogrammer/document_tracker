import 'dart:io';
import 'package:com.cpdco.docutracker/screens/add_locational_zoning_screen.dart';
...
class LocalationalZoningScreen extends StatefulWidget {
  ...
  const LocalationalZoningScreen({
    super.key,
  ...
  @override
  State<LocalationalZoningScreen> createState() => _LocalationalZoningScreenState();
}

class _LocalationalZoningScreenState extends State<LocalationalZoningScreen> {
  ...
  void _filterDocuments() {
    setState(() {
      _filteredDocuments = widget.documents.where((doc) {
        if (doc.mode != 'Locational & Zoning') return false;
  ...
  Widget build(BuildContext context) {
    return ConnectivityBanner(
      child: Scaffold(
        floatingActionButton: FloatingActionButton(
          onPressed: () async {
            await Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => const AddLocationalZoningScreen()),
            );
        ...
        appBar: AppBar(
          title: const Text("Locational & Zoning"),
          backgroundColor: const Color(0xFF00ACC1),
        ...
        body: RefreshIndicator(
          ...
          child: Container(
            ...
            child: _isLoading
                ...
                ? const Center(
                    child: Text(
                      'No Locational & Zoning records found',
                              ...
                              placeholder: (context, url) => const Center(
                                child: Column(
                                  ...
                                  children: [
                                    CircularProgressIndicator(),
                                    SizedBox(height: 16),
                                    Text('wait la po...', style: TextStyle(color: Colors.black, fontSize: 16)),
                                  ],
                                ),
                              ),
                                ...
                                : () async {
                                    ...
                                    try {
                                      ...
                                      if (context.mounted) {
                                        ...
                                        if (context.mounted) setState(() => isSuccess = false);
                                      }
                                    } catch (e) {
                                      if (context.mounted) {
                      ...
                      Container(
                        ...
                        child: Center(
                          child: AnimatedSwitcher(
                            ...
                            child: isDownloading
                                ? const CircularProgressIndicator(key: ValueKey('loading'), color: Colors.white)
                                : Column(
                                    mainAxisSize: MainAxisSize.min,
                                    children: const [
                                      Icon(Icons.check, color: Colors.green, size: 48),
                                      SizedBox(height: 8),
                                      Text('Saved!', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600)),
                                    ],
                                  ),
                          ),
                        ),
  ...
  void _showFileDialog(BuildContext context, Document document) {
    ...
    for (int i = 0; i < document.fileUrls.length; i++) {
      ...
      allNames.add(i < document.fileNames.length
          ? document.fileNames[i]
          : document.fileUrls[i].split('/').last.split('\\').last);
    }
    showDialog(
      context: context,
        ...
        builder: (context, dialogSetState) => AlertDialog(
              ...
              children: [
                ...
                TextField(
                  ...
                  onTap: () async {
                    ...
                    if (picked != null) {
                      dialogSetState(() {});
                      setState(() { _specificDate = picked; _startDate = picked; _endDate = picked; });
                    }
                  },
                ),
                const SizedBox(height: 16),
                TextField(
                  ...
                  onTap: () async {
                    ...
                    if (picked != null) {
                      dialogSetState(() {});
                      setState(() { _startDate = picked; _specificDate = null; });
                    }
                  },
                ),
                const SizedBox(height: 16),
                TextField(
                  ...
                  onTap: () async {
                    ...
                    if (picked != null) {
                      dialogSetState(() {});
                      setState(() { _endDate = picked; _specificDate = null; });
                    }
                  },
                ),
              ],
          ...
          actions: [
            TextButton(
              onPressed: () {
                dialogSetState(() {});
                setState(() { _specificDate = null; _startDate = null; _endDate = null; });
                _filterDocuments();
                Navigator.pop(context);
              },
            ...
            ElevatedButton.icon(
              ...
              onPressed: () { _filterDocuments(); Navigator.pop(context); },
            ),
          ],
        ),
      ),
  ...
  String? _extractFileId(String url) {
    if (url.contains('drive.google.com')) {
      ...
      if (url.contains('/file/d/')) {
        ...
        if (fileIndex != -1 && fileIndex + 1 < segments.length) return segments[fileIndex + 1];
