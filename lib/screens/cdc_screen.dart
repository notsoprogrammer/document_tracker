import 'dart:io';
import 'package:com.cpdco.docutracker/screens/add_cdc_screen.dart';
...
class _CdcScreenState extends State<CdcScreen> {
      ...
      children: [
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(label, style: TextStyle(fontWeight: FontWeight.w500, color: Theme.of(context).colorScheme.primary, fontSize: 12)),
    ...
    if (hasUploads) {
      return Container(
        ...
        child: Row(
          ...
          children: [
            SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, valueColor: AlwaysStoppedAnimation<Color>(Colors.orange))),
            const SizedBox(width: 8),
            Text('Uploading $uploadedFiles/$totalFiles files...', style: TextStyle(fontSize: 12, color: Colors.orange[700], fontWeight: FontWeight.w500)),
          ],
        ),
      );
    }
    ...
    return ConnectivityBanner(
      child: Scaffold(
        floatingActionButton: FloatingActionButton(
          onPressed: () async {
            await Navigator.push(context, MaterialPageRoute(builder: (context) => const AddCdcScreen()));
        ...
        appBar: AppBar(
          title: const Text("CDC Documents"),
          backgroundColor: const Color(0xFFFF7043),
          ...
          child: Container(
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: [Theme.of(context).colorScheme.surface, Theme.of(context).colorScheme.surfaceVariant.withOpacity(0.3)],
              ),
            ),
            child: _isLoading
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        if (document.description != null && document.description!.isNotEmpty) ...[
                                          ...
                                          _buildDetailRow(Icons.notes, "Description", document.description!),
                                        ],
                                        const SizedBox(height: 8),
                                        ...
                                        const SizedBox(height: 8),
                                        _buildDetailRow(Icons.access_time, "Timestamp", document.createdAt != null ? _formatDateTime(document.createdAt!) : 'Unknown'),
                                          ...
                                          GestureDetector(
                                            ...
                                            child: Row(
                                              children: [
                                                ...
                                                const SizedBox(width: 8),
                                                Expanded(child: Text(document.referenceLink!, style: const TextStyle(color: Colors.blue, decoration: TextDecoration.underline), overflow: TextOverflow.ellipsis)),
                                              ],
                                            ),
                                          ),
                                        ],
                                          ...
                                          children: [
                                            if (_username == document.person)
                                              ElevatedButton.icon(
                                                ...
                                                onPressed: () {
                                                  Navigator.push(context, MaterialPageRoute(builder: (context) => EditDocumentScreen(document: document))).then((_) { if (widget.onRefresh != null) widget.onRefresh!(); });
                                                },
                                                style: ElevatedButton.styleFrom(backgroundColor: const Color.fromARGB(255, 78, 127, 218), minimumSize: const Size(48, 40), padding: const EdgeInsets.only(left: 10), alignment: Alignment.center, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
                                              ),
                                            if (_username == document.person)
                                              ElevatedButton(
                                                style: ElevatedButton.styleFrom(backgroundColor: const Color.fromARGB(255, 218, 87, 78), foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
                                                onPressed: () async {
                                                  final deleted = await confirmAndDeleteRecord(context, document, CachedDocumentService());
                                                  if (deleted && mounted) {
                                            ...
                                            if (document.imageUrls.isNotEmpty || document.localImagePaths.isNotEmpty)
                                              ElevatedButton(
                                                ...
                                                style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
                                            ...
                                            if (document.filePath != null || document.fileUrls.isNotEmpty)
                                              ElevatedButton(
                                                onPressed: () {
                                                  ...
                                                  if (allFiles.length == 1) _viewFile(allFiles[0]); else _showFileDialog(context, document);
                                                },
                                                style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
  ...
  Future<void> _shareDocument(Document doc) async {
    ...
    try {
      ...
      Future<List<int>?> fetchImageViaProxy(String imageUrl) async {
        try {
          final fileId = imageUrl.contains('drive.google.com/uc?id=') ? Uri.parse(imageUrl).queryParameters['id'] ?? imageUrl : imageUrl;
      ...
      for (final fileUrl in doc.fileUrls) { final link = toViewLink(fileUrl); if (link != null) pdfLinks.add(link); }
      if (!mounted) return;
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      final clipboardText = pdfLinks.isEmpty ? title : '$title\n\n${pdfLinks.join('\n')}';
      await Clipboard.setData(ClipboardData(text: clipboardText));
      if (mounted) {
        final msg = pdfLinks.isEmpty ? 'Title copied to clipboard — paste it as your message in Messenger' : 'PDF links copied to clipboard — paste them in Messenger after sending the images';
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(msg), duration: const Duration(seconds: 5)));
      }
      if (xFiles.isNotEmpty) { await Share.shareXFiles(xFiles, subject: title); }
      else if (pdfLinks.isNotEmpty) { await Share.share(clipboardText, subject: title); }
      else { Share.share(title, subject: title); }
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
                        ...
                        return InteractiveViewer(
                          child: Center(
                            child: CachedNetworkImage(
                              ...
                              fit: BoxFit.contain,
                              placeholder: (context, url) => const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [CircularProgressIndicator(), SizedBox(height: 16), Text('wait la po...', style: TextStyle(color: Colors.black, fontSize: 16))])),
                        ...
                        children: [
                          IconButton(
                            ...
                            onPressed: isDownloading ? null : () async {
                              setState(() { isDownloading = true; isSuccess = false; });
                              try {
                                await ImageDownloadService.downloadAndSave(imageUrls[currentIndex]);
                                if (context.mounted) { setState(() { isDownloading = false; isSuccess = true; }); await Future.delayed(const Duration(milliseconds: 1500)); if (context.mounted) setState(() => isSuccess = false); }
                              } catch (e) { if (context.mounted) { setState(() => isDownloading = false); SnackbarUtils.showErrorSnackBar(context, e.toString().replaceAll('Exception: ', '')); } }
                            },
  ...
  void _showFileDialog(BuildContext context, Document document) {
    ...
    for (int i = 0; i < document.fileUrls.length; i++) {
      ...
      allNames.add(i < document.fileNames.length ? document.fileNames[i] : document.fileUrls[i].split('/').last.split('\\').last);
    }
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Select File to View'),
        content: SizedBox(width: double.maxFinite, child: ListView.builder(shrinkWrap: true, itemCount: allFiles.length, itemBuilder: (context, index) => ListTile(leading: const Icon(Icons.attach_file), title: Text(allNames[index]), onTap: () { Navigator.pop(context); _viewFile(allFiles[index]); }))),
    ...
    showDialog(
      ...
      builder: (_) => StatefulBuilder(
        builder: (context, dialogSetState) => AlertDialog(
          ...
          title: Row(children: [Icon(Icons.filter_list, color: Theme.of(context).colorScheme.primary), const SizedBox(width: 8), const Text("Filter Documents", style: TextStyle(fontSize: 16))]),
  ...
  String? _extractFileId(String url) {
    if (url.contains('drive.google.com')) {
      ...
      if (url.contains('/file/d/')) { final segments = uri.pathSegments; final fileIndex = segments.indexOf('d'); if (fileIndex != -1 && fileIndex + 1 < segments.length) return segments[fileIndex + 1]; }
      else if (url.contains('uc?id=')) { return uri.queryParameters['id']; }
    }
    if (RegExp(r'^[a-zA-Z0-9_-]{20,}$').hasMatch(url)) return url;
    return null;
  }
  ...
  void _viewFile(String filePath) async {
    try {
      ...
      final urlToLaunch = kIsWeb ? 'https://drive.google.com/uc?id=$fileId&export=download' : 'https://drive.google.com/file/d/$fileId/view?usp=sharing';
